...
func _physics_process(delta: float) -> void:
	...
	if moving_backward and platform.position.direction_to(start_point.position) == Vector2.ZERO:
...
func check_player_climb():
	...
	if player_on_side != null:
		if player_on_side.current_state == player_on_side.STATES.climb and !started_moving:
			moving_forward = true
			FmodBanks.start.play()
			started_moving = true
